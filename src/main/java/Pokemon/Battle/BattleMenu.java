...
public class BattleMenu {
    ...
    public void draw(Canvas canvas, BattleState battleState) {
        switch (battleState) {
            case SELECT_ACTION:
                canvas.drawString("1. Fight", 600, 500, Color.WHITE); //600 400
                canvas.drawString("2. Bag", 600, 510, Color.WHITE); // 600 410
                canvas.drawString("3. Pokemon", 600, 520, Color.WHITE); // 600 420
                canvas.drawString("4. Run", 600, 530, Color.WHITE); // 600 430
                break;
            case SELECT_MOVE:
                for (int i = 0; i < actions.length; i++) {
                    ...
                    canvas.drawString((i + 1) + ". " + actions[i].getName(), 200, 500 + (i * 10), actions[i].getRemainingPP() > 0 ? Color.WHITE : Color.RED);
                }
                break;
            case SELECT_ITEM:
